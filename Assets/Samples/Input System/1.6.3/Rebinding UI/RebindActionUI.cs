...
/// <summary>
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public bool ResolveActionAndBinding(out InputAction action, out int bindingIndex)
        {
            ...
            {
                default:
                case BindingModifier.Binding.Forward:
                    action = playerInputActionsSO.Value.PlayerActionMap.Move;
                    bindingIndex = 1;
                    break;
                case BindingModifier.Binding.Backward:
                    action = playerInputActionsSO.Value.PlayerActionMap.Move;
                    bindingIndex = 2;
                    break;
                case BindingModifier.Binding.Left:
                    action = playerInputActionsSO.Value.PlayerActionMap.Move;
                    bindingIndex = 3;
                    break;
                case BindingModifier.Binding.Right:
                    action = playerInputActionsSO.Value.PlayerActionMap.Move;
                    bindingIndex = 4;
                    break;
                case BindingModifier.Binding.Jump:
                    action = playerInputActionsSO.Value.PlayerActionMap.Jump;
                    bindingIndex = isGamepad ? 1 : 0;
                    break;
                case BindingModifier.Binding.Throw:
                    action = playerInputActionsSO.Value.PlayerActionMap.Throw;
                    bindingIndex = isGamepad ? 1 : 0;
                    break;
                case BindingModifier.Binding.Pause:
                    action = playerInputActionsSO.Value.PlayerActionMap.Pause;
                    bindingIndex = isGamepad ? 1 : 0;
                    break;
            }
        ...
        public void StartInteractiveRebind()
        {
            ...
            if (action.bindings[bindingIndex].isComposite)
            {
                ...
                if (firstPartIndex >= action.bindings.Count ||
                    !action.bindings[firstPartIndex].isPartOfComposite) return;
                if (isGamepad)
                {
                    PerformInteractiveGamepadRebind(action, bindingIndex, allCompositeParts: true);
                }
                else
                {
                    PerformInteractiveKBMRebind(action, bindingIndex, allCompositeParts: true);
                }
            }
            else
            {
                if (isGamepad)
                {
                    PerformInteractiveGamepadRebind(action, bindingIndex);
                }
                else
                {
                    PerformInteractiveKBMRebind(action, bindingIndex);
                }
            }
        }

        private void PerformInteractiveGamepadRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if (CheckDuplicateBindings(action, bindingIndex, allCompositeParts))
                        {
                            action.RemoveBindingOverride(bindingIndex);
                            CleanUp();
                            PerformInteractiveGamepadRebind(action, bindingIndex, allCompositeParts);
                            return;
                        }
                        ...
                        if (allCompositeParts)
                        {
                            ...
                            if (nextBindingIndex < action.bindings.Count && action.bindings[nextBindingIndex].isPartOfComposite)
                                PerformInteractiveGamepadRebind(action, nextBindingIndex, true);
                        }
                    });
        ...
        private void PerformInteractiveKBMRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            m_RebindOperation?.Cancel(); // Will null out m_RebindOperation.
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if (CheckDuplicateBindings(action, bindingIndex, allCompositeParts))
                        {
                            action.RemoveBindingOverride(bindingIndex);
                            CleanUp();
                            PerformInteractiveKBMRebind(action, bindingIndex, allCompositeParts);
                            return;
                        }
                        ...
                        if (allCompositeParts)
                        {
                            var nextBindingIndex = bindingIndex + 1;
                            if (nextBindingIndex < action.bindings.Count && action.bindings[nextBindingIndex].isPartOfComposite)
                                PerformInteractiveKBMRebind(action, nextBindingIndex, true);
                        }
                    });
        ...
        [SerializeField]
        private PlayerInputActionsSO playerInputActionsSO;
        
        [SerializeField]
        private BindingModifier.Binding binding;
