...
/// <summary>
/// Manages the list of players displayed in the voice chat interface whilst the player is in the 'InLobby' scene.
/// 
/// @author Vivox sample package
/// @date May 2024
/// </summary>
public class InLobbyRosterManager : MonoBehaviour
{
    ...
    [SerializeField] private DictWithStringVivoxPartListSO vivoxPartsDict;
    
    private void Awake()
    {
        vivoxPartsDict.InitialiseValue();
    ...
    void OnParticipantAdded(VivoxParticipant participant)
    {
        ...
        vivoxPartsDict.Value.Add(participant);
    }
    ...
    void OnParticipantRemoved(VivoxParticipant participant)
    {
        if (m_RosterObjects.ContainsKey(participant.ChannelName))
        {
            ...
            if (removedItem != null)
            {
                ...
                vivoxPartsDict.Value.Remove(participant);
            }
            else
            {
